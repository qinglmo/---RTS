...
public class MovementRules
{
    ...
    public static bool IsWalkable_Static(Vector2Int cell, BaseUnit self = null)//提供静态地图通行判断,忽略动态单位
    {
        // 1. 边界检查
        if (!GridManager.BoundsChecking(cell))
            return false;
        if(GridManager.IsOccupied_Static(cell))
            return false;
        // 5. 无任何阻挡 → 可通行
        return true;
    }
    public static bool IsOccupyable(Vector2Int cell, IFactionMember self = null)
    {
        // 1. 边界检查
        if (!GridManager.BoundsChecking(cell))
            return false;
        ...
        {
            //自身占领认为可占领
            if (self != null && GridManager.GetFactionMember(cell) == self)
                return true;
            return false;
        }
        // 5. 无任何阻挡 → 可占领
    ...
    /// <returns></returns>
    public static bool IsPreOccupyable(Vector2Int cell, IFactionMember self)//是否可以预占领，是否可以抵达。
    {
    ...
    /// <param name="self"></param>
    /// <returns></returns>
    private static bool IsFactionOccupy(Vector2Int cell, IFactionMember self)//被占领了返回true,没被占领返回false
    {
