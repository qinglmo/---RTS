...
public class GridManager : MonoBehaviour, IOccupyGridManager, ISnapToGrid, IGridPassableCheck, IGridEntityProvider, IRegisterEntity
{
    ...
    public bool IsOccupied_Static(Vector2Int gridPos)//提供静态地图占用判断,忽略动态单位
    {
        if(!BoundsChecking(gridPos))
            return false;
        if(occupiedEnityInCells[gridPos.x, gridPos.y] != null)
        {
            if(occupiedEnityInCells[gridPos.x, gridPos.y] is BaseBuilding)
                return true;
            if(occupiedEnityInCells[gridPos.x, gridPos.y] is Mountain)
                return true;
        }
        return false;
    }
