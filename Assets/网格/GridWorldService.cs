...
public class GridWorldService : MonoBehaviour, IMoveApplyWorld
{
    ...
    public void Initialize(IOccupyGridManager gridOccupancy, IGridEntityProvider gridEntityProvider, IGridPassableCheck gridPassableCheck)
    {
    ...
    public bool TryOccupy(BaseUnit unit,Vector2Int start ,Vector2Int target)//该方法可以自动管理noOccupyUnits,只需要单位正确调用即可
    {
        //位置为空，则可通行
        if (!gridPassableCheck.IsOccupied(target))
        {
    ...
    private void UpdatePhysicalEasy()
    {
        ...
        foreach(var unit in noOccupyUnits)
        {
            ...
            if (unit.movement.stepMover.stateMachine.CurrentStateEnum==StepMover.MainState.Nothing)
            {
                toPush.Add(unit);
                continue;
            }
        ...
        foreach(var unit in toPush)
        {
            ...
            if (target!=null)
            {
                unit.movement.stepMover.PushTo((Vector2Int)target);//执行弹开成功移除noOccupyUnits
            }
        }
    }
    private void UpdatePhysicalEasyAll()
    {
        ...
        foreach (var unit in allUnit)
        {
            if (noOccupyUnits.Contains(unit)||unit.movement.stepMover.direction!=Vector2.zero
                ...
                || unit.movement.stepMover.stateMachine.CurrentStateEnum == StepMover.MainState.MovingCenter)
                continue;
